<%-- 
    Document   : errorExterno
    Created on : 27-jun-2023, 17:06:41
    Author     : hoshi
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="paginas/stylesErrorEx.css"/>
        <link rel="icon" href="star.ico" type="image/x-icon">
        <title>Error</title>
    </head>
    <body>
        <div class="container">

            <div  class="error">

                <h2>${error}</h2>
                <p class="p">4</p>
                <span class="dracula">			
                    <div class="con">
                        <div class="hair"></div>
                        <div class="hair-r"></div>
                        <div class="head"></div>
                        <div class="eye"></div>
                        <div class="eye eye-r"></div>
                        <div class="mouth"></div>
                        <div class="blod"></div>
                        <div class="blod blod2"></div>
                    </div>
                </span>
                <p class="p">4</p>

                <div class="page-ms">
                    <p class="page-msg"> Oops, la página que buscas Desapareció </p>
                    <a href="login.jsp"><button class="go-back">Regresar</button></a>
                </div>
            </div>
        </div>

        <iframe style="width:0;height:0;border:0; border:none;" scrolling="no" frameborder="no" allow="autoplay" src="https://instaud.io/_/2Vvu.mp3"></iframe>
    </body>
</html>
